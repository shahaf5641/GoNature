...
/**
 * this class handles all mysql queries that related to orders
 */
public class OrderQueries {
	...
	/**
	 * This function retrieve from the database all the orders that related to given time period.
	 * 
	...
	public ArrayList<Order> getOrderBetweenTimes(ArrayList<?> parameters) {
		...
		try {
			...
			query.setString(7, OrderStatusName.PENDING_24_HOURS_BEFORE.toString());
			query.setString(8, OrderStatusName.IN_WAITING_LIST.toString());
	...
	public boolean addNewOrder(Object obj) {
		...
		try {
			...
			query.setString(7, orderToAdd.getTravelerEmail());
			query.setDouble(8, orderToAdd.getTotalPrice());
	...
	public ArrayList<Order> findMatchingOrdersInWaitingList(ArrayList<?> parameters) {
		...
		try {
			...
			query.setString(5, OrderStatusName.IN_WAITING_LIST.toString());
	...
	public ArrayList<Order> getOrderWithExpiryAlertTime(String date, String time) {
		...
		try {
			...
			query.setString(3, OrderStatusName.PENDING_24_HOURS_BEFORE.toString());
			query.setString(4, OrderStatusName.IN_WAITING_LIST.toString());
	...
	public ArrayList<Order> getWaitingOrdersToCancel() {
		...
		try {
			query = conn.prepareStatement(sql);
			query.setString(1, OrderStatusName.IN_WAITING_LIST.toString());
			query.setString(2, LocalDate.now().toString());
